...
#include "ui/logo_metrics.h"      // where the art sits inside aiohud_logo.raw (GENERATED by scripts/gen_logo.py)
...
void ConfigPage::draw(const Frame& f, float sw, float sh) {
    ...
    ensure_raw_tex_mip(dev, logoTex_, logoRetry_, LOGO_PATH(), LOGO_TEX_W, LOGO_TEX_H);
    ...
    if (logoTex_) {
        ...
        { const float per = 7.0f; float lp = f.t / per; lp -= floorf(lp);
          if (GLEAM_ON && lp < 0.42f) {
              ...
              // and produced the milky film. gen_logo.py floods the opaque colours outward now, so there is
