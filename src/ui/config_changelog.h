...
static const ChangeLine CL_75[] = {
    ...
    { "*The distance to a party member no longer freezes when they run far away.* Past about 50 yalms the game stops tracking them, and their last position stayed on screen as a normal-looking number that never changed again. Their line now shows *50+* in red instead, and goes back to the exact distance the moment they are back in range.",
      "*La distance d'un membre du groupe ne se fige plus quand il s'eloigne.* Au-dela d'environ 50 yalms le jeu cesse de le suivre, et sa derniere position restait affichee comme un chiffre normal qui ne bougeait plus jamais. Sa ligne affiche desormais *50+* en rouge, et retrouve la distance exacte des qu'il revient a portee." },
    { "*Buffs you cast on allies stop disappearing over a long session.* The list of what each member is carrying had a fixed number of slots that were never freed, and trusts take a new one every time you re-summon them. Once it filled up, most members' rows were simply not drawn and their expiry alerts went silent.",
      "*Les buffs que vous lancez sur vos allies ne disparaissent plus au fil d'une longue session.* La liste de ce que porte chaque membre avait un nombre de places fixe, jamais liberees, et les trusts en prennent une nouvelle a chaque re-invocation. Une fois pleine, les lignes de la plupart des membres n'etaient tout simplement plus dessinees et leurs alertes d'expiration devenaient muettes." },
    { "*Expiry alerts no longer stay on screen for a category you turned off*, and watching allies during an alliance no longer uses up the alert slots your own buffs need.",
      "*Les alertes d'expiration ne restent plus affichees pour une categorie que vous avez desactivee*, et le suivi des allies en alliance ne consomme plus les places dont vos propres buffs ont besoin." },
    { "*Limbus remembers your run across a plugin reload*, and your coffer history is no longer wiped by an update. The runs-left count and the quadrant chips come back instead of showing \"?\".",
      "*Limbus se souvient de votre run apres un rechargement du plugin*, et votre historique de coffres n'est plus efface par une mise a jour. Le nombre de runs restants et les pastilles de quadrant reviennent au lieu d'afficher \" ? \"." },
    { "*Omen shows its bonus objectives instead of hiding them.* On the first floor the box had read the objectives correctly and then dropped them, until the game happened to repeat its floor announcement. Treasure floors are also recognised whatever way the game spells them.",
      "*Omen affiche ses objectifs bonus au lieu de les masquer.* Au premier etage la boite lisait pourtant correctement les objectifs puis les jetait, jusqu'a ce que le jeu repete son annonce d'etage. Les etages a coffres sont aussi reconnus quelle que soit la facon dont le jeu les ecrit." },
    { "*Text stays sharp after you open the settings.* Opening the panel over a visible HUD used up every cached text size, and everything drawn afterwards was stretched from the nearest one instead. Map and party icons were also slightly softened by the same kind of leftover.",
      "*Le texte reste net apres l'ouverture des reglages.* Ouvrir le panneau par-dessus un HUD visible consommait toutes les tailles de texte en cache, et tout ce qui etait dessine ensuite etait etire depuis la plus proche. Les icones de la carte et du groupe etaient adoucies par un residu du meme genre." },
    { "*Deleting the profile you are using now really deactivates it.* It stayed marked as active, and saving afterwards silently recreated it. A profile that cannot be written to disk now says so instead of pretending it was saved.",
      "*Supprimer le profil que vous utilisez le desactive vraiment.* Il restait marque comme actif, et enregistrer ensuite le recreait en silence. Un profil qui ne peut pas etre ecrit sur le disque le dit desormais, au lieu de faire croire qu'il a ete enregistre." },
    { "*Updating with two clients open no longer reports a false failure.* Both would install at once and the loser overwrote the winner's result, so you were told the update had failed when it had actually worked. Only one installs now. An update that really fails also leaves a log next to the plugin, so it can be diagnosed.",
      "*Mettre a jour avec deux clients ouverts ne signale plus un faux echec.* Les deux installaient en meme temps et le perdant ecrasait le resultat du gagnant, donc on vous annoncait un echec alors que tout avait fonctionne. Un seul installe desormais. Une mise a jour qui echoue vraiment laisse aussi un journal a cote du plugin, pour pouvoir etre diagnostiquee." },
    { "Monster ability names were out of date : several showed the wrong name and a few recent ones were missing entirely.",
      "Les noms de capacites de monstres etaient perimes : plusieurs affichaient un nom errone et quelques recents manquaient completement." },
    { "Under the hood : the window skin, the settings and several readings of game memory now report a failure instead of failing silently, and the build no longer produces any warning at all -- so a new one cannot hide among the old.",
      "Sous le capot : l'habillage des fenetres, les reglages et plusieurs lectures de la memoire du jeu signalent desormais un echec au lieu d'echouer en silence, et la compilation ne produit plus aucun avertissement -- ainsi un nouveau ne peut plus se cacher parmi les anciens." },
};
static const ChangeLine CL_74[] = {
    { "*An update that fails can no longer leave you without the plugin.* The new build was unpacked straight over your Windower folder, and that operation deletes each file before rewriting it -- then undoes itself if anything goes wrong along the way. So a single file it could not write, anywhere in the fourteen hundred that ship, could end with no AioHud at all : the HUD closed, and Windower answered \"File does not exist\" when it tried to come back. The download is now verified and unpacked aside first, while the HUD is still running ; your folder is only touched once the new build is known to be complete, and the plugin itself is replaced last, from a copy that is put back if that step fails. Whatever happens, a failed update now leaves your install exactly as it was.",
      "*Une mise a jour qui echoue ne peut plus vous laisser sans le plugin.* La nouvelle version etait decompressee directement par-dessus votre dossier Windower, et cette operation supprime chaque fichier avant de le reecrire -- puis defait tout si quoi que ce soit se passe mal en chemin. Un seul fichier impossible a ecrire, parmi les quatorze cents livres, pouvait donc ne laisser aucun AioHud : le HUD se fermait, et Windower repondait \"File does not exist\" au moment de le relancer. Le telechargement est desormais verifie et decompresse a cote d'abord, pendant que le HUD tourne encore ; votre dossier n'est touche qu'une fois la nouvelle version reconnue complete, et le plugin lui-meme est remplace en dernier, a partir d'une copie qui est restauree si cette etape echoue. Quoi qu'il arrive, une mise a jour ratee laisse maintenant votre installation exactement dans l'etat ou elle etait." },
    { "*And it now tells you why.* The reason an update did not go through was written to a file nobody reads : all you saw was the HUD closing and an error from Windower that named nothing. It is now said in chat, along with what to do about it.",
      "*Et elle vous dit desormais pourquoi.* La raison pour laquelle une mise a jour n'est pas passee etait ecrite dans un fichier que personne ne lit : vous ne voyiez que le HUD se fermer et une erreur de Windower qui ne nommait rien. Elle est maintenant annoncee dans le chat, avec la marche a suivre." },
    { "A plugin that is missing while its folder is still there is now reported as an install to repair, instead of being taken for a deliberate removal -- which used to make the updater quietly take its own startup line back out.",
      "Un plugin absent dont le dossier est encore la est desormais signale comme une installation a reparer, au lieu d'etre pris pour une suppression volontaire -- ce qui amenait la mise a jour a retirer discretement sa propre ligne de demarrage." },
};
static const ChangeLine CL_73[] = {
    { "*A job ability no longer opens a skillchain with a weaponskill's properties.* Shield Bash and Expiacion carry the same internal number, and so do Weapon Bash and Ruinator, Jump and Gale Axe : using Shield Bash opened a resonance window under Expiacion's element, so the closer it suggested was wrong. They are told apart by what the game announces, which is the only thing that differs between them -- and it was measured on eleven weaponskills rather than assumed. The weaponskill popup already knew this rule ; the Skillchains box did not.",
      "*Une capacite de job n'ouvre plus un skillchain avec les proprietes d'une attaque speciale.* Shield Bash et Expiacion portent le meme numero interne, comme Weapon Bash et Ruinator, ou Jump et Gale Axe : utiliser Shield Bash ouvrait une fenetre de resonance sous l'element d'Expiacion, et la cloture proposee etait donc fausse. Ils sont desormais distingues par ce que le jeu annonce, la seule chose qui differe entre eux -- et cela a ete mesure sur onze attaques speciales, pas suppose. Le popup d'attaque speciale connaissait deja cette regle ; la boite Skillchains non." },
    { "*The Limbus weekly allowance no longer vanishes when you leave the zone.* How many data collections you have left in the week is account-wide, shared between Apollyon and Temenos, but it was only remembered while you stayed inside -- stepping out and coming back showed nothing. It is now kept with the date it was seen : a count from an earlier week reports a full allowance again after the Sunday reset instead of a stale number, and a count that has never been seen shows a question mark rather than a plausible guess.",
      "*L'allocation hebdomadaire de Limbus ne disparait plus quand vous quittez la zone.* Le nombre de collectes de donnees qu'il vous reste dans la semaine est lie au compte, partage entre Apollyon et Temenos, mais il n'etait retenu que le temps ou vous restiez a l'interieur -- sortir et revenir n'affichait plus rien. Il est desormais conserve avec la date a laquelle il a ete vu : un compte d'une semaine precedente redonne une allocation complete apres la reinitialisation du dimanche, au lieu d'un nombre perime, et un compte jamais observe affiche un point d'interrogation plutot qu'une valeur plausible." },
    { "*Omen : the bonus objectives of each floor are listed, with their progress.* They are read from what the game announces, floor by floor. A floor carrying fewer objectives than the previous one no longer leaves the extra lines of the old floor behind, and boss or treasure floors correctly show none.",
      "*Omen : les objectifs bonus de chaque etage sont listes, avec leur progression.* Ils sont lus depuis ce que le jeu annonce, etage par etage. Un etage comportant moins d'objectifs que le precedent ne laisse plus trainer les lignes en trop de l'ancien etage, et les etages de boss ou de tresor n'en affichent aucun, correctement." },
};
...
static const ChangeLine CL_71[] = {
    ...
    { "*Removing AioHud no longer leaves Windower loading something that is not there.* The plugin adds a line to Windower's startup file so its updater loads with the game, and nothing ever removed it : deleting AioHud left Windower trying to load a missing add-on at every launch, forever, with no way to tell where it came from. The companion add-on now takes its own line back out when it sees the plugin is gone.",
      "*Desinstaller AioHud ne laisse plus Windower charger quelque chose qui n'existe plus.* Le plugin ajoute une ligne au fichier de demarrage de Windower pour que son updater se charge avec le jeu, et rien ne l'enlevait jamais : supprimer AioHud laissait Windower tenter de charger un add-on absent a chaque lancement, indefiniment, sans moyen de savoir d'ou ca venait. L'add-on compagnon retire desormais sa propre ligne quand il constate que le plugin n'est plus la." },
    { "*Resizing a box is smooth again.* Every single frame of a slider drag was rewriting your whole settings file and, for a box placed by the layout, rebuilding every element of the HUD -- reloading its images and re-reading the zone map along the way. The box still follows the slider exactly as before ; the saving and the rebuilding now happen once, when you stop moving it.",
      "*Redimensionner une boite est de nouveau fluide.* Chaque image d'un deplacement de curseur reecrivait tout votre fichier de reglages et, pour une boite placee par la mise en page, reconstruisait chaque element du HUD -- en rechargeant ses images et en relisant la carte de zone au passage. La boite suit toujours le curseur exactement comme avant ; l'enregistrement et la reconstruction n'ont plus lieu qu'une fois, quand vous arretez de la bouger." },
    { "*The diagnostic log no longer erases itself, and no longer balloons.* It was wiped every time the plugin was loaded -- so unloading and reloading to see if a problem went away destroyed the record of that very problem, and so did every update. It is kept now, with the version and the time at the top of each session. A diagnostic line that was meant to appear a handful of times per session was also repeating on every frame once it had seen enough different cases : one real session had produced a 22 MB file.",
      "*Le journal de diagnostic ne s'efface plus, et ne gonfle plus.* Il etait vide a chaque chargement du plugin -- donc decharger et recharger pour voir si un probleme avait disparu detruisait la trace de ce probleme, et chaque mise a jour faisait de meme. Il est desormais conserve, avec la version et l'heure en tete de chaque session. Une ligne de diagnostic censee apparaitre quelques fois par session se repetait par ailleurs a chaque image des qu'elle avait vu assez de cas differents : une session reelle avait produit un fichier de 22 Mo." },
    { "Two smaller display fixes : the bright edge at the tip of the target's HP bar no longer disappears when the bar is set very thin, and the Minimap samples in the Help tab no longer stay blank for the rest of the session if their images happen to miss on the first try.",
      "Deux corrections d'affichage plus modestes : le lisere clair au bout de la barre de vie de la cible ne disparait plus quand la barre est reglee tres fine, et les apercus de Minicarte de l'onglet Aide ne restent plus vides pour le reste de la session si leurs images echouent au premier essai." },
};
