...
static const HelpItem HELP_GENERAL[] = {
    ...
    {2, "*//aio out alerts* removes only the rows in red (the buffs that just ran out), leaving the live timers alone.",
        "*//aio out alertes* retire seulement les lignes en rouge (les buffs qui viennent d'expirer), sans toucher aux minuteurs en cours."},
    {2, "*//aio in* puts back every line you removed that way (a name or a number puts back just one).",
        "*//aio in* remet toutes les lignes retirees de cette facon (un nom ou un numero n'en remet qu'une)."},
...
static const HelpItem HELP_TIMERS[] = {
    ...
    {2, "A Duration row can show the *icon*, the *name*, or *both* — and a row about *someone else* always names them : *Icon* is icon + person, *Name* is person + spell, *Both* is all three. An ally row never turns into an anonymous icon. Recast rows are always the *name* (no icon set exists for recasts).",
        "Une ligne Duration peut montrer l'*icône*, le *nom*, ou les *deux* — et une ligne qui concerne *quelqu'un d'autre* le nomme toujours : *Icône* donne icône + personne, *Nom* personne + sort, *Les deux* les trois. Une ligne alliée ne devient jamais une icône anonyme. Les lignes Recast sont toujours le *nom* (pas de jeu d'icônes pour les recasts)."},
    ...
    {2, "*Layout* is Fused or Separate. *Duration: show* picks icon, name or both for the Duration column — on a row about someone else the *person is always named*, so *Icon* is icon + person, *Name* is person + spell, *Both* is all three. *Box / Frame* sets the theme and can turn the frame off. *Size* scales the whole box.",
        "*Disposition* est Fusionné ou Séparé. *Duration : afficher* choisit icône, nom ou les deux pour la colonne Duration — sur une ligne qui concerne quelqu'un d'autre, la *personne est toujours nommée* : *Icône* donne icône + personne, *Nom* donne personne + sort, *Les deux* donne les trois. *Box / Cadre* règle le thème et peut couper le cadre. *Taille* met tout le cadre à l'échelle."},
    ...
    {2, "*Cast a buff on the wrong person and you can take it back.* Every watched row carries a small number on its left ; type *//aio out <number>* and that line disappears — name, timer and its future red alert with it. The buff itself is untouched : what goes is our decision to watch it. The number belongs to the *row*, not to its position, so it is still the right one by the time you have finished typing.",
        "*Un buff lancé sur la mauvaise personne peut être annulé.* Chaque ligne suivie porte un petit numéro à gauche ; tape *//aio out <numéro>* et la ligne disparaît — nom, minuteur et sa future alerte rouge avec. Le buff, lui, n'est pas touché : ce qui part, c'est la décision de le surveiller. Le numéro appartient à la *ligne*, pas à sa position, donc c'est encore le bon quand tu as fini de taper."},
    {2, "*//aio out <name>* or *<spell>* works just as well, both together in either order (*//aio out gab haste*). *//aio out list* prints everything being watched, numbers included — the one way to reach a row the column is not showing (clipped by Max per column, or hidden by a filter).",
        "*//aio out <nom>* ou *<sort>* marche aussi bien, les deux ensemble dans l'ordre que tu veux (*//aio out gab haste*). *//aio out list* affiche tout ce qui est suivi, numéros compris — le seul moyen d'atteindre une ligne que la colonne ne montre pas (coupée par Max par colonne, ou cachée par un filtre)."},
    {2, "*//aio out alerts* removes only the *red* rows — the buffs that have just run out — and leaves every running timer alone. That is the everyday one, after a wipe or a long fight. *//aio out all* clears the lot, red or not.",
        "*//aio out alertes* ne retire que les lignes *rouges* — les buffs qui viennent d'expirer — et laisse tourner tous les minuteurs. C'est celle du quotidien, après une wipe ou un long combat. *//aio out all* efface tout, rouge ou pas."},
    {2, "*A line is removed for that one cast, not forever.* If the buff ends, the row is forgotten for good and *no red alert fires* — which is exactly why you removed it. If you *cast that spell on that person again*, the row comes back watched : that cast you meant, so its expiry is worth knowing. It comes back even when they still carried the buff — Haste overwrites itself and never lapses. A cast that does not land (no effect, resisted) changes nothing and leaves the row off.",
        "*Une ligne est retirée pour ce cast-là, pas pour toujours.* Si le buff se termine, la ligne est oubliée pour de bon et *aucune alerte rouge* ne se déclenche — c'est précisément pour ça que tu l'as retirée. Si tu *relances ce sort sur cette personne*, la ligne revient sous surveillance : ce cast-là tu l'as voulu, donc son expiration t'intéresse. Elle revient même si la personne avait toujours le buff — Haste s'écrase lui-même et ne tombe jamais. Un cast qui ne prend pas (sans effet, résisté) ne change rien et laisse la ligne retirée."},
    {2, "*//aio in* puts back every line you removed, right now ; a number or a name puts back just one (*//aio in 3*, *//aio in gab*). A row that is still running comes back with its real remaining time — removing a line never touched the timer.",
        "*//aio in* remet tout ce que tu as retiré, tout de suite ; un numéro ou un nom n'en remet qu'une (*//aio in 3*, *//aio in gab*). Une ligne qui tourne encore revient avec son temps restant réel — retirer une ligne n'a jamais touché au minuteur."},
    ...
    {1, "Single-target buffs (Haste, Protect) you spread over several allies show *one row per person* -- which is also the only form that can name who is missing one. A row folds into a single \"(AoE N)\" line only when the cast really did hit N people: a song, Protectra, a COR roll, or a spell made area-wide by *SCH Accession / Manifestation*. Rows are ordered *what you cast first, then buffs with no known caster, then other players, then trusts*, and finally AoE groups and each ally. Within every block, the soonest to expire comes first.",
        "Les buffs monocibles (Haste, Protect) que tu répartis sur plusieurs alliés s'affichent *une ligne par personne* — la seule forme qui puisse aussi te dire QUI n'en a pas. Une ligne ne se replie en un \"(AoE N)\" unique que si le cast a réellement touché N personnes : une chanson, un Protectra, un roll COR, ou un sort passé en zone par *SCH Accession / Manifestation*. Les lignes sont ordonnées *ce que tu as lancé d'abord, puis les buffs sans lanceur connu, puis les autres joueurs, puis les trusts*, et enfin les groupes AoE et chaque allié. Dans chaque bloc, le plus proche d'expirer passe en premier."},
    {2, "A *grouped* row is about several people at once, so it names none of them and follows *Duration: show* on its own — in *Icon* it is one icon, with the \"(AoE N)\" mark riding along with the name.",
        "Une ligne *groupée* concerne plusieurs personnes à la fois : elle n'en nomme aucune et suit *Duration : afficher* de bout en bout — en *Icône*, c'est une icône seule, la marque \"(AoE N)\" partant avec le nom."},
