...
static const HelpItem HELP_TIMERS[] = {
    ...
    {1, "Each buff carries a *dot*. Click it to cycle through four behaviours — the legend just above the list shows each one. The cycle is *Tracked → Tracked + focus → Hidden → Hidden + focus* and back.",
        "Chaque buff porte une *pastille*. Clique dessus pour parcourir quatre comportements — la légende juste au-dessus de la liste les montre. Le cycle est *Suivi → Suivi + focus → Masqué → Masqué + focus* puis retour."},
    ...
    {2, "*Hidden + focus* : hidden while healthy, but *pops a red alert* once it drops under the *Focus: warn under* threshold, or when it falls / is dispelled. The alert holds for the *Focus: hold alert* duration then clears itself if you don't re-cast. Both are sliders in the *Alerts* section. Zero clutter, zero misses.",
        "*Masqué + focus* : caché tant que tout va bien, mais *surgit en alerte rouge* dès qu'il passe sous le seuil *Focus : alerte sous*, ou qu'il tombe / est dispel. L'alerte tient le temps réglé par *Focus : garder alerte* puis s'efface si tu ne relances pas. Les deux sont des curseurs dans la section *Alertes*. Zéro encombrement, zéro oubli."},
