...
| what | source |
|---|---|
| key items owned | [`owns_key_item(id)`](../player/key-items.md) — the flat `u8[8192]` at `*(g+0x4C)` |
| items owned | [`count_item(id)` / `count_items`](../player/inventory.md) — all 18 bags at `*(g+0x50)` |
...
## See also
- [Key items](../player/key-items.md) — the `u8[8192]` read behind `owns_key_item`.
- [Inventory](../player/inventory.md) — the 18-bag `count_item` read.
