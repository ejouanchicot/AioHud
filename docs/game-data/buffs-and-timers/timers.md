...
with the VARIABLE stride (see [`action-packet.md`](../actions/action-packet.md) — the fixed `150 + i*123` stride
...
- [PointWatch — XP/CP/ML + Merits](../player/pointwatch.md) — shares the 0x063 Set-Update packet (orders 2 / 5 vs order 9 here).
- [Party cast bar — 0x028](../party/cast-bar.md) — the ActionPacket bit layout the buff-caster filter reads (actor.param @bit 86, cats 4/6).
- [Party-member buffs — 0x076](member-buffs.md) — the buff status-icon atlas the Duration column reuses.
- [Skillchains](../actions/skillchains.md) — the same `on_action` cat-4/6 parse feeds the buff-caster records.
