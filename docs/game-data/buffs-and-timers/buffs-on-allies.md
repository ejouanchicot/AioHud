...
- **augment%** = `enh_dur_augment_pct` — extdata **0x4E0** decode (`% = value + 1`, see
  [Player equipment](../player/player-equipment.md)) + `ENH_DUR_PATH` Delve-neck path augments. **All jobs.**
...
- [Player equipment](../player/player-equipment.md) — the equipped-item ids + extdata the augment decode reads.
- [Party cast bar — 0x028](../party/cast-bar.md) — the ActionPacket bit layout (actor.param @bit 86, target stride 123).
